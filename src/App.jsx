...
const AIEngine = {
  ...
  analyze: (campaign) => {
    ...
    if (ins.spend > 150 && ins.conversions === 0) {
      issues.push({ severity: 'critical', icon: 'flame', title: 'Gasto sem conversões', desc: `${fmt.money(ins.spend)} investidos sem resultado. Pause imediatamente.` });
    }
    ...
    if (ins.cpa > 0 && ins.cpa < cfg.metaCPA * 0.6 && ins.conversions >= 2) {
      opportunities.push({ icon: 'rocket', title: 'Pronta para escalar', desc: `CPA ${Math.round((1 - ins.cpa / cfg.metaCPA) * 100)}% abaixo da meta. Aumente o orçamento!` });
    }
...
:root {
  ...
  --accent-primary-muted: rgba(16, 185, 129, 0.12);
...
.animate-slide { animation: slideUp 0.25s ease forwards; }
...
.ai-badge { background: var(--accent-primary); color: white; font-size: 9px; font-weight: 700; padding: 2px 6px; border-radius: 4px; letter-spacing: 0.5px; }
...
.campaign-main {
  display: grid;
  grid-template-columns: 56px 1fr 95px 95px 95px 110px;
  align-items: center;
  gap: 18px;
  padding: 14px 18px;
  cursor: pointer;
...
.campaign-status { display: inline-flex; align-items: center; gap: 4px; padding: 2px 7px; border-radius: var(--radius-sm); font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.3px; }
...
.breakdown-label { font-size: 10px; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px; }
...
@media (max-width: 1400px) {
  ...
  .campaign-main { grid-template-columns: 50px 1fr 85px 85px 100px; }
}
...
// Auth Form
function AuthForm({ isRegister, onSubmit, onToggle, loading }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    isRegister ? onSubmit(name, email, password) : onSubmit(email, password);
  };
    ...
    <form onSubmit={handleSubmit}>
      {isRegister && (
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: 'block', fontSize: 12, color: 'var(--text-secondary)', marginBottom: 6 }}>Nome</label>
      ...
      <div style={{ marginBottom: 22 }}>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--text-secondary)', marginBottom: 6 }}>Senha</label>
      ...
      <p style={{ textAlign: 'center', marginTop: 18, fontSize: 13, color: 'var(--text-muted)' }}>
        ...
        <span onClick={onToggle} style={{ color: 'var(--accent-primary)', cursor: 'pointer', fontWeight: 500 }}>
...
// Connect Prompt
function ConnectPrompt({ token, setToken, onConnect, loading }) {
  return (
    <div style={{ maxWidth: 440, margin: '50px auto', textAlign: 'center' }}>
      <div style={{ width: 70, height: 70, background: 'var(--accent-info-muted)', borderRadius: 'var(--radius-lg)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 22px' }}>
        <Icon name="zap" size={36} style={{ color: 'var(--accent-info)' }} />
      </div>
      ...
      <div style={{ textAlign: 'left', marginBottom: 14 }}>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--text-secondary)', marginBottom: 6 }}>Token de Acesso</label>
        <input className="input" type="text" value={token} onChange={(e) => setToken(e.target.value)} placeholder="Cole seu token do Meta aqui" />
        <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 6 }}>Obtenha em: developers.facebook.com - Graph API Explorer</p>
      </div>
      <button className="btn btn-primary" style={{ width: '100%', padding: 12 }} onClick={onConnect} disabled={loading || !token.trim()}>
...
function CampaignRow({ campaign, expanded, onToggle, onAction, activeTab, setActiveTab }) {
  ...
  const getCpaClass = () => {
    if (!ins.cpa || ins.cpa === 0) return 'neutral';
    if (ins.cpa <= AIEngine.config.metaCPA * 0.7) return 'good';
    if (ins.cpa <= AIEngine.config.metaCPA * 1.2) return 'warning';
    return 'bad';
  };
  
  const getRoasClass = () => {
    if (!ins.roas || ins.roas === 0) return 'neutral';
    if (ins.roas >= AIEngine.config.metaROAS) return 'good';
    if (ins.roas >= AIEngine.config.metaROAS * 0.7) return 'warning';
    return 'bad';
  };
    ...
    <div className={`campaign-row ${expanded ? 'expanded' : ''}`}>
      <div className="campaign-main" onClick={onToggle}>
        ...
        <div className="metric-cell">
          <div className="metric-value">{fmt.moneyCompact(ins.spend || 0)}</div>
          <div className="metric-label">Gasto</div>
        </div>
        
        <div className="metric-cell">
          <div className={`metric-value ${getCpaClass()}`}>{ins.cpa ? fmt.money(ins.cpa) : '-'}</div>
          <div className="metric-label">CPA</div>
        </div>
        
        <div className="metric-cell">
          <div className={`metric-value ${getRoasClass()}`}>{ins.roas ? `${ins.roas.toFixed(2)}x` : '-'}</div>
          <div className="metric-label">ROAS</div>
        </div>
        
        <div className="campaign-actions" onClick={(e) => e.stopPropagation()}>
          {score >= 75 && ins.conversions > 0 && (
            <button className="btn-icon scale" title="Escalar 30%" onClick={() => onAction('scale', campaign.id)}>
              <Icon name="rocket" size={16} />
            </button>
          )}
          {isActive ? (
            <button className="btn-icon warning" title="Pausar" onClick={() => onAction('pause', campaign.id)}>
              <Icon name="pause" size={16} />
            </button>
          ) : (
            <button className="btn-icon scale" title="Ativar" onClick={() => onAction('activate', campaign.id)}>
          ...
          <div className="expanded-content">
              ...
              <>
                <div className="score-breakdown">
                  <div className="breakdown-item">
                    <div className="breakdown-value" style={{ color: AIEngine.getStatus(campaign.score?.breakdown?.cpa || 0).color }}>{campaign.score?.breakdown?.cpa || 0}</div>
                    <div className="breakdown-label">CPA Score</div>
                  </div>
                  <div className="breakdown-item">
                    <div className="breakdown-value" style={{ color: AIEngine.getStatus(campaign.score?.breakdown?.ctr || 0).color }}>{campaign.score?.breakdown?.ctr || 0}</div>
                    <div className="breakdown-label">CTR Score</div>
                  </div>
                  <div className="breakdown-item">
                    <div className="breakdown-value" style={{ color: AIEngine.getStatus(campaign.score?.breakdown?.freq || 0).color }}>{campaign.score?.breakdown?.freq || 0}</div>
                    <div className="breakdown-label">Frequência</div>
                  </div>
                  <div className="breakdown-item">
                    <div className="breakdown-value" style={{ color: AIEngine.getStatus(campaign.score?.breakdown?.roas || 0).color }}>{campaign.score?.breakdown?.roas || 0}</div>
                ...
                <div className="issues-grid">
                  {campaign.analysis?.issues?.map((issue, i) => (
                    <div key={i} className={`issue-card ${issue.severity}`}>
                      <div className="issue-icon"><Icon name={issue.icon} size={18} /></div>
                  ...
                  {campaign.analysis?.opportunities?.map((opp, i) => (
                    <div key={i} className="issue-card opportunity">
                      <div className="issue-icon"><Icon name={opp.icon} size={18} /></div>
                      <div className="issue-content">
                        <div className="issue-title">{opp.title}</div>
                        <div className="issue-desc">{opp.desc}</div>
                      </div>
                    </div>
                  ))}
                  {!campaign.analysis?.issues?.length && !campaign.analysis?.opportunities?.length && (
                    <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: 20, color: 'var(--text-muted)' }}>
                      Nenhum insight detectado para esta campanha.
                    </div>
                  )}
                </div>
              </>
            ...
            {activeTab === 'metrics' && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
                <div className="breakdown-item">
                  <div className="breakdown-value">{fmt.num(ins.impressions || 0)}</div>
                  <div className="breakdown-label">Impressões</div>
                </div>
                <div className="breakdown-item">
                  <div className="breakdown-value">{fmt.num(ins.clicks || 0)}</div>
                  <div className="breakdown-label">Cliques</div>
                </div>
                <div className="breakdown-item">
                  <div className="breakdown-value">{fmt.pct(ins.ctr || 0)}</div>
                  <div className="breakdown-label">CTR</div>
                </div>
                <div className="breakdown-item">
                  <div className="breakdown-value">{(ins.frequency || 0).toFixed(2)}</div>
                  <div className="breakdown-label">Frequência</div>
                </div>
                <div className="breakdown-item">
                  <div className="breakdown-value">{fmt.num(ins.reach || 0)}</div>
                  <div className="breakdown-label">Alcance</div>
                </div>
                <div className="breakdown-item">
                  <div className="breakdown-value">{ins.conversions || 0}</div>
                  <div className="breakdown-label">Conversões</div>
                </div>
                <div className="breakdown-item">
                  <div className="breakdown-value">{fmt.money(ins.cpm || 0)}</div>
                  <div className="breakdown-label">CPM</div>
                </div>
                <div className="breakdown-item">
                  <div className="breakdown-value">{fmt.money(ins.spend || 0)}</div>
                  <div className="breakdown-label">Gasto Total</div>
                </div>
              </div>
            )}
              ...
              <>
                <div className="budget-control">
                  <div className="budget-header">
                    <span className="budget-title">Controle de Orçamento</span>
                    <span className="budget-current">Atual: <span>{fmt.money(campaign.daily_budget / 100 || campaign.lifetime_budget / 100 || 0)}</span>/dia</span>
                  </div>
                  <div className="budget-actions">
                    <button className="budget-btn increase" onClick={() => onAction('scale', campaign.id)}>
                      <Icon name="arrowUp" size={16} />
                      Aumentar 30%
                    </button>
                    <button className="budget-btn decrease" onClick={() => onAction('reduce', campaign.id)}>
                ...
                <div style={{ marginTop: 16, display: 'flex', gap: 10 }}>
                  {isActive ? (
                    <button className="btn btn-danger" onClick={() => onAction('pause', campaign.id)}>
                      <Icon name="pause" size={16} />
                      Pausar Campanha
                    </button>
                  ) : (
                    <button className="btn btn-primary" onClick={() => onAction('activate', campaign.id)}>
                      <Icon name="play" size={16} />
                      Ativar Campanha
                    </button>
                  )}
...
export default function App() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (campRes.success) {
        const enriched = (campRes.campaigns || []).map(c => ({
          ...c,
  ...
  const handleConnect = async () => {
    ...
    if (res.success) {
      ...
      setConnected(true);
      setSuccess('Meta conectado com sucesso!');
      loadAccounts();
  ...
  const handleAction = async (action, campaignId) => {
    ...
    setLoading(false);
    if (res.success) {
      setSuccess(action === 'pause' ? 'Campanha pausada' : action === 'activate' ? 'Campanha ativada' : action === 'scale' ? 'Orçamento aumentado' : 'Orçamento reduzido');
      loadData();
    } else setError(res.error || 'Erro ao executar ação');
  };
  ...
  const stats = useMemo(() => {
    const t = campaigns.reduce((acc, c) => {
      ...
      return { spend: acc.spend + (ins.spend || 0), conversions: acc.conversions + (ins.conversions || 0), clicks: acc.clicks + (ins.clicks || 0), impressions: acc.impressions + (ins.impressions || 0) };
  ...
  const filterCounts = useMemo(() => ({
    ...
    active: campaigns.filter(c => c.effectiveStatus === 'ACTIVE').length,
    paused: campaigns.filter(c => c.effectiveStatus === 'PAUSED').length,
    critical: campaigns.filter(c => c.score?.total < 40).length,
    scale: campaigns.filter(c => c.score?.total >= 75 && c.insights?.conversions > 0).length,
  }), [campaigns]);
        ...
        <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-base)', padding: 20 }}>
          <div className="animate-fade" style={{ width: '100%', maxWidth: 380, background: 'var(--bg-subtle)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-xl)', padding: '40px 32px' }}>
            <div style={{ textAlign: 'center', marginBottom: 32 }}>
              <div className="logo-mark" style={{ width: 60, height: 60, margin: '0 auto 16px', borderRadius: 14 }}>
            ...
            {error && (
              <div style={{ background: 'var(--accent-danger-muted)', border: '1px solid rgba(239,68,68,0.25)', color: 'var(--accent-danger)', padding: '10px 14px', borderRadius: 'var(--radius-md)', marginBottom: 18, fontSize: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                <Icon name="alertCircle" size={16} />{error}
              </div>
            )}
            <AuthForm isRegister={page === 'register'} onSubmit={page === 'register' ? handleRegister : handleLogin} onToggle={() => setPage(page === 'login' ? 'register' : 'login')} loading={loading} />
          </div>
        </div>
      </>
    );
        ...
        <aside className="sidebar">
          <div className="sidebar-header">
            <div className="sidebar-logo">
              <div className="logo-mark"><Icon name="brain" size={20} /></div>
              <div><span className="logo-title">AdBrain</span><br/><span className="logo-subtitle">AI-Powered</span></div>
            </div>
          </div>
        ...
        <main className="main-content">
          <header className="header">
            <div className="header-left">
              <div>
                <h1 className="header-title">{page === 'campaigns' ? 'Campanhas' : page === 'dashboard' ? 'Dashboard' : page === 'settings' ? 'Configurações' : page === 'audience' ? 'Análise de Público' : page === 'insights' ? 'Insights IA' : 'Criativos'}</h1>
                <p className="header-subtitle">{page === 'campaigns' ? 'Gerencie e otimize suas campanhas' : page === 'dashboard' ? 'Visão geral da conta' : page === 'settings' ? 'Configure sua conta' : 'Análise inteligente'}</p>
              </div>
            </div>
            <div className="header-right">
              ...
              <button className="btn btn-secondary" onClick={loadData} disabled={loading}>
                <Icon name="refreshCw" size={15} className={loading ? 'animate-spin' : ''} />Atualizar
              </button>
            </div>
          </header>
          ...
          {(error || success) && (
            <div className="toast">
              <div className={`toast-content ${error ? 'error' : 'success'}`}>
                <Icon name={error ? 'alertCircle' : 'checkCircle'} size={16} />{error || success}
              </div>
            </div>
          )}
          ...
          <div className="page-content">
            {!connected ? (
              <ConnectPrompt token={token} setToken={setToken} onConnect={handleConnect} loading={loading} />
            ) : page === 'campaigns' ? (
              <>
                {/* Stats */}
                <div className="stats-grid">
                  <div className="stat-card">
                    <div className="stat-header"><div className="stat-icon blue"><Icon name="dollarSign" size={18} /></div></div>
                    <div className="stat-value">{fmt.moneyCompact(stats.spend)}</div>
                  ...
                  <div className="stat-card">
                    <div className="stat-header"><div className="stat-icon yellow"><Icon name="target" size={18} /></div></div>
                    <div className="stat-value">{stats.cpa > 0 ? fmt.money(stats.cpa) : '-'}</div>
                    <div className="stat-label">CPA Médio</div>
                  </div>
                  <div className="stat-card">
                    <div className="stat-header"><div className="stat-icon red"><Icon name="activity" size={18} /></div></div>
                    <div className="stat-value">{fmt.pct(stats.ctr)}</div>
                ...
                {(summary.critical > 0 || summary.scalable > 0) && (
                  <div className="ai-summary">
                    ...
                    <div className="ai-content">
                      ...
                      <p className="ai-text">
                        {summary.critical > 0 && <><strong className="danger">{summary.critical} campanha{summary.critical > 1 ? 's' : ''} crítica{summary.critical > 1 ? 's' : ''}</strong> precisam de atenção imediata, desperdiçando {fmt.money(summary.wastedSpend)}. </>}
                        {summary.scalable > 0 && <><strong className="success">{summary.scalable} campanha{summary.scalable > 1 ? 's' : ''}</strong> {summary.scalable > 1 ? 'estão prontas' : 'está pronta'} para escalar.</>}
                      </p>
                      <div className="ai-actions">
                        {summary.critical > 0 && <button className="btn btn-danger btn-sm" onClick={() => setFilter('critical')}><Icon name="alertTriangle" size={14} />Ver Críticas</button>}
                        {summary.scalable > 0 && <button className="btn btn-primary btn-sm" onClick={() => setFilter('scale')}><Icon name="rocket" size={14} />Ver para Escalar</button>}
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="filter-bar">
                  ...
                  <div className="search-box">
                    ...
                    <input className="search-input" type="text" placeholder="Buscar campanha..." value={search} onChange={(e) => setSearch(e.target.value)} />
                  </div>
                </div>
                ...
                <div className="campaigns-list">
                  {filteredCampaigns.length === 0 ? (
                    <div className="empty-state">
                      <Icon name="target" size={48} className="empty-icon" />
                      <h3 className="empty-title">Nenhuma campanha encontrada</h3>
                      <p className="empty-text">Ajuste os filtros ou aguarde os dados carregarem.</p>
                    </div>
                  ) : (
              ...
              <div className="settings-grid">
                <div className="settings-card">
                  <h3 className="settings-card-title"><Icon name="link" size={18} />Conexão Meta Business</h3>
                  <div className="settings-row">
                    <span className="settings-label">Status</span>
                    <span className={`connection-badge ${connected ? 'connected' : 'disconnected'}`}>
                      <span className="connection-dot"></span>{connected ? 'Conectado' : 'Desconectado'}
                    </span>
                  </div>
                  {connected && (
                    <div style={{ marginTop: 16 }}>
                      <button className="btn btn-danger" onClick={handleDisconnect}><Icon name="x" size={16} />Desconectar</button>
                    </div>
                  )}
                </div>
                <div className="settings-card">
                  <h3 className="settings-card-title"><Icon name="sliders" size={18} />Metas de Performance</h3>
                  <div className="settings-row">
                    <span className="settings-label">Meta de CPA</span>
                    <span className="settings-value">{fmt.money(AIEngine.config.metaCPA)}</span>
                  </div>
                  <div className="settings-row">
                    <span className="settings-label">Meta de ROAS</span>
                    <span className="settings-value">{AIEngine.config.metaROAS}x</span>
                  </div>
                  <div className="settings-row">
                    <span className="settings-label">CTR Mínimo</span>
                    <span className="settings-value">{AIEngine.config.ctrMinimo}%</span>
                  </div>
                  <div className="settings-row">
                    <span className="settings-label">Frequência Máxima</span>
                    <span className="settings-value">{AIEngine.config.frequenciaMaxima}x</span>
                  </div>
                </div>
                <div className="settings-card">
                ...
                <div className="settings-card">
                  <h3 className="settings-card-title"><Icon name="briefcase" size={18} />Conta de Anúncios</h3>
                  <div className="settings-row">
                  ...
                  <div className="settings-row">
                    <span className="settings-label">Total de Contas</span>
                    <span className="settings-value">{accounts.length}</span>
                  </div>
                </div>
              </div>
            ) : page === 'dashboard' ? (
              <>
                <div className="stats-grid">
                  ...
                  <div className="stat-card">
                    <div className="stat-header"><div className="stat-icon red"><Icon name="barChart3" size={18} /></div></div>
                    <div className="stat-value">{fmt.numCompact(stats.clicks)}</div>
                ...
                <div className="settings-grid" style={{ marginTop: 8 }}>
                  <div className="settings-card">
                    <h3 className="settings-card-title"><Icon name="target" size={18} />Campanhas por Status</h3>
                    <div className="settings-row">
                      <span className="settings-label">Total de Campanhas</span>
                      <span className="settings-value">{campaigns.length}</span>
                    </div>
                    <div className="settings-row">
                      <span className="settings-label">Ativas</span>
                      <span className="settings-value" style={{ color: 'var(--accent-primary)' }}>{filterCounts.active}</span>
                    </div>
                    <div className="settings-row">
                      <span className="settings-label">Pausadas</span>
                      <span className="settings-value" style={{ color: 'var(--text-muted)' }}>{filterCounts.paused}</span>
                    </div>
                    <div className="settings-row">
                      <span className="settings-label">Críticas</span>
                      <span className="settings-value" style={{ color: 'var(--accent-danger)' }}>{filterCounts.critical}</span>
                    </div>
                    <div className="settings-row">
                      <span className="settings-label">Prontas para Escalar</span>
                      <span className="settings-value" style={{ color: 'var(--accent-primary)' }}>{filterCounts.scale}</span>
                    </div>
                  </div>
                  <div className="settings-card">
                    <h3 className="settings-card-title"><Icon name="activity" size={18} />Métricas Gerais</h3>
                    <div className="settings-row">
                      <span className="settings-label">Impressões</span>
                      <span className="settings-value">{fmt.numCompact(stats.impressions)}</span>
                    </div>
                    <div className="settings-row">
                      <span className="settings-label">Cliques</span>
                      <span className="settings-value">{fmt.numCompact(stats.clicks)}</span>
                    </div>
                    <div className="settings-row">
                      <span className="settings-label">CTR Médio</span>
                      <span className="settings-value">{fmt.pct(stats.ctr)}</span>
                    </div>
                    <div className="settings-row">
                      <span className="settings-label">Conversões</span>
                      <span className="settings-value">{stats.conversions}</span>
                    </div>
                    <div className="settings-row">
                      <span className="settings-label">CPA Médio</span>
                      <span className="settings-value">{stats.cpa > 0 ? fmt.money(stats.cpa) : '-'}</span>
                    </div>
                  </div>
                </div>
              </>
              ...
              <>
                <div className="settings-grid">
                  <div className="settings-card">
                    ...
                    {breakdown.gender?.map((g, i) => (
                      <div className="settings-row" key={i}>
                        <span className="settings-label">{g.gender === 'male' ? 'Masculino' : g.gender === 'female' ? 'Feminino' : g.gender}</span>
                        <span className="settings-value">{g.conversions || 0} conv. | {fmt.money(g.spend || 0)}</span>
                      </div>
                    )) || <div style={{ color: 'var(--text-muted)', fontSize: 13 }}>Sem dados disponíveis</div>}
                  </div>
                  <div className="settings-card">
                    <h3 className="settings-card-title"><Icon name="barChart3" size={18} />Por Idade</h3>
                    {breakdown.age?.map((a, i) => (
                      <div className="settings-row" key={i}>
                        <span className="settings-label">{a.age}</span>
                        <span className="settings-value">{a.conversions || 0} conv. | {fmt.money(a.spend || 0)}</span>
                      </div>
                    )) || <div style={{ color: 'var(--text-muted)', fontSize: 13 }}>Sem dados disponíveis</div>}
                  </div>
                  <div className="settings-card">
                    <h3 className="settings-card-title"><Icon name="target" size={18} />Por Posicionamento</h3>
                    {breakdown.placement?.slice(0, 6).map((p, i) => (
                      <div className="settings-row" key={i}>
                  ...
                  <div className="settings-card">
                    <h3 className="settings-card-title"><Icon name="smartphone" size={18} />Por Dispositivo</h3>
                    {breakdown.device?.map((d, i) => (
                      <div className="settings-row" key={i}>
                        <span className="settings-label">{d.device_platform || d.impression_device || 'Desconhecido'}</span>
                        <span className="settings-value">{d.conversions || 0} conv. | {fmt.money(d.spend || 0)}</span>
                      </div>
                    )) || <div style={{ color: 'var(--text-muted)', fontSize: 13 }}>Sem dados disponíveis</div>}
                  </div>
                </div>
              ...
              <>
                <div className="ai-summary" style={{ marginBottom: 24 }}>
                  <div className="ai-icon"><Icon name="sparkles" size={22} /></div>
                  <div className="ai-content">
                    <div className="ai-header"><span className="ai-title">Análise Inteligente da Conta</span><span className="ai-badge">IA</span></div>
                    <p className="ai-text">
                      {campaigns.length === 0 ? (
                        'Carregue suas campanhas para receber insights personalizados.'
                      ) : (
                        <>
                          Você tem <strong>{campaigns.length} campanhas</strong> ativas na conta. 
                          {summary.critical > 0 && <> <strong className="danger">{summary.critical}</strong> precisam de atenção urgente.</>}
                          {summary.scalable > 0 && <> <strong className="success">{summary.scalable}</strong> estão com performance excelente e podem ser escaladas.</>}
                          {summary.critical === 0 && summary.scalable === 0 && ' Todas as campanhas estão com performance estável.'}
                        </>
                      )}
                    </p>
                  </div>
                ...
                <div className="issues-grid" style={{ marginBottom: 28 }}>
                  {campaigns.flatMap(c => (c.analysis?.issues || []).map((issue, i) => (
                    <div key={`${c.id}-${i}`} className={`issue-card ${issue.severity}`}>
                      ...
                      <div className="issue-content">
                        <div className="issue-title">{issue.title}</div>
                        <div className="issue-desc"><strong>{c.name}:</strong> {issue.desc}</div>
                      </div>
                    </div>
                  ))).slice(0, 6)}
                  {campaigns.every(c => !c.analysis?.issues?.length) && (
                    <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: 30, color: 'var(--text-muted)' }}>
                      ...
                      <div>Nenhum problema crítico detectado nas suas campanhas.</div>
                    </div>
                  )}
                </div>

                <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 16 }}>Oportunidades de Crescimento</h3>
                <div className="issues-grid">
                  {campaigns.flatMap(c => (c.analysis?.opportunities || []).map((opp, i) => (
                    <div key={`${c.id}-opp-${i}`} className="issue-card opportunity">
                      <div className="issue-icon"><Icon name={opp.icon} size={18} /></div>
                      <div className="issue-content">
                        <div className="issue-title">{opp.title}</div>
                        <div className="issue-desc"><strong>{c.name}:</strong> {opp.desc}</div>
                      </div>
                    </div>
                  ))).slice(0, 6)}
                  {campaigns.every(c => !c.analysis?.opportunities?.length) && (
                    <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: 30, color: 'var(--text-muted)' }}>
                      <Icon name="lightbulb" size={40} style={{ marginBottom: 12, opacity: 0.5 }} />
                      <div>Melhore a performance das campanhas para desbloquear oportunidades de escala.</div>
                    </div>
                  )}
                </div>
              </>
              ...
              <div className="empty-state">
                ...
                <h3 className="empty-title">Em breve</h3>
                <p className="empty-text">Esta seção está em desenvolvimento.</p>
              </div>
            )}
          </div>
        </main>
